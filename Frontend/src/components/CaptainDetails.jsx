...
function CaptainDetails() {
	...
	const [captainTodayDetails, setCaptainTodayDetails] = useState(null);
	const token = localStorage.getItem("token");

	const fetchCaptainData = async () => {
		try {
			const response = await toast.promise(
				axios.get(`${import.meta.env.VITE_BASE_URL}/captains/todaysDetails`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				}),
				{
					loading: "Fetching captains today's data. Please wait...",
					success: "Fetching data successful!",
					error: "Error during fetching captainTodayData",
				}
			);

			if (response.status === 200) {
				const data = response.data.message;
				setCaptainTodayDetails(data);
			}
		} catch (error) {
		...
		<div className="h-3/6 px-4 bg-white shadow-lg rounded-t-3xl">
			...
			<div className="flex items-center justify-between gap-4 mb-6">
				...
				<div className="text-center bg-green-50 px-4 py-2 rounded-lg">
					<h4 className="text-lg font-bold text-green-700">₹ {captainTodayDetails?.fare}</h4>
			...
			<div className="bg-gray-50 rounded-xl p-6 shadow-inner">
				<div className="flex items-center justify-center gap-6 px-7">
					...
					<div className="flex-1 text-center bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
						...
						<h4 className="text-lg font-bold">{(captainTodayDetails?.duration / 60).toFixed(1)}</h4>
					...
					<div className="flex-1 text-center bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
						...
						<h4 className="text-lg font-bold">{captainTodayDetails?.distance.toFixed(2)}</h4>
					...
					<div className="flex-1 text-center bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
						...
						<h4 className="text-lg font-bold">{captainTodayDetails?.ridesDone}</h4>
