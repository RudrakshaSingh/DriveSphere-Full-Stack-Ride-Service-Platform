          ...
          ₹{Math.max(
            props.fare[props.vehicleType] -
              (props.couponResponse.type === "fixed"
                ...
                : (props.fare[props.vehicleType] * props.couponResponse.discount) / 100),
            0
          )}
        </p>
      </div>
		  ...
		  <div className="fixed top-0 left-0 w-full h-full bg-black/50 backdrop-blur-sm flex items-center justify-center">
			<div className="bg-white rounded-xl p-6 shadow-lg">
			  ...
			  <div className="flex justify-end gap-4">
				<button className="py-2 px-4 rounded-md text-gray-600 hover:text-gray-800" onClick={() => setShowCouponPopup(false)}>
				  Cancel
				</button>
				<button className="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition-colors" onClick={handleCouponSubmit}>
				  Apply
				</button>
			  </div>
			</div>
